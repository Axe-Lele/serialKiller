...
public class LaboratoryManager : Singleton<LaboratoryManager>
{
	...
	public void SelectMatchedPopup()
	{
		...
		if (m_MatchedItemList.Count == 0)
		{
			m_AlarmEmptyLabel.gameObject.SetActive(true);
			m_MatchedResultLabel.text = string.Empty;
			for (int i = 0; i < m_MatchedList.Length; i++)
			{
				m_MatchedList[i].Reset();
			}
	...
	public void CheckMatched(int time)
	{
		if (LaboratoryDataManager.instance.m_IsStartMatched)
		{
			...
			if (LaboratoryDataManager.instance.m_MatchedRemainTime > 0)
			{
				...
				m_RemainMatchedTimeLabel.text = string.Format("{0:00}:{1:00}", h, m);
			}
			else
			{
				...
				string str = string.Format(Localization.Get("System_Text_CompleteMatched"), LaboratoryDataManager.instance.ReturnMatchedIndexList(0), LaboratoryDataManager.instance.ReturnMatchedIndexList(1));
	...
	public void RemoveMatchedItem(int i)
	{
		if (LaboratoryDataManager.instance.m_IsStartMatched)
			return;

		m_MatchedList[i].Reset();
		LaboratoryDataManager.instance.SetMatchedIndexList(i, "");
	}

	public void AddMatchedItemList(string str)
	{
	...
	{
		string str = "";
		if (m_MatchedResultLabel.text != null)
		{
			str = m_MatchedResultLabel.text;
		}
