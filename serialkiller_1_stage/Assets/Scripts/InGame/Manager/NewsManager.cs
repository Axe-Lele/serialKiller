...
{
	private readonly static string NewsTitleFormat = "News_{0}_{1}_{2}_Title";
	public static string GetNewsTitle(string itemCode)
	{
		return Localization.Get(
			string.Format(NewsTitleFormat, PlayDataManager.instance.m_StageName,
	...
	/// <summary>
	/// 읽지 않은 뉴스를 새롭게 추가합니다.
	/// 현재 노출되고 있는 뉴스가 없다면, 추가한 뉴스를 노출시킵니다.
	/// </summary>
	...
	public void AddUnreadNewsList(int date, string index)
	{
		...
		{
			if (m_NewsItemList[i].m_ItemCode == index)
				return;
	...
	[SerializeField]
	private float m_SwapTimer = 5f;
	/// <summary>
	/// SwapTimer에 따라 현재 보여지고 있는 뉴스를 계속 변경합니다.
	/// </summary>
	/// <returns></returns>
	private IEnumerator View()
	{
		...
		m_ViewTitleLabel.text = Localization.Get("News_" + PlayDataManager.instance.m_StageName + "_" + StageDataManager.instance.m_CriminalCode + "_" + m_NewsItemList[m_ProgressIndex].m_ItemCode + "_Title");
	...
	public void ShowNews(int index)
	{
		...
		NewsDataManager.instance.MoveNewsItem(m_NewsItemList[index].m_Date, m_NewsItemList[index].m_ItemCode);
		...
		m_ProgressIndex = m_NewsItemList.Count - 1;
